/**
 * Applies references/sql/alter/2026-09-10_add_kpi_published_notification_type.sql
 * — widens employee_notifications.type CHECK to allow `kpi.published` — then
 * verifies it landed.
 *
 *   node scripts/apply-kpi-published-notification-type.mjs           # apply + verify
 *   node scripts/apply-kpi-published-notification-type.mjs --verify  # verify only
 *
 * Until this runs, the "Accounting: a dept-week's KPI bonuses were published"
 * notification does not exist: every insert is rejected by the CHECK. `kpi.scored`
 * shipped dead exactly that way for three days. Verify, don't assume.
 *
 ...
 * shape as scripts/apply-hours-gap-notification-type.mjs.
 *
...
const SQL_PATH = "references/sql/alter/2026-09-10_add_kpi_published_notification_type.sql";
const NEW_TYPES = ["kpi.published"];
