...
export const PASS_DATE = '2026-08-26';
export const AUDIT_RANGE = '667dfe9d..HEAD';
export const AUDIT_COMMITS = 9;
...
export const ROWS: PassRow[] = [
  // EVERY ROW IN THIS PASS IS Pending Deploy, AND NOT ONE IS Done. The push landed 2026-08-26
  // (667dfe9d..854a78b8, 9 commits), which removed the ONLY blocker the previous staging carried -
  // "not an ancestor of origin/main" - so four rows advance exactly one step. Vercel deploying is
  // not somebody having looked, so none of them advances two. Done is a separate question, asked of
  // Kane in the review and answered by him, never inferred here.
  {
    name: 'Kolan showed a fallback monogram for a day because /kolan.png was referenced and never existed',
    status: 'Done',
    completed: '2026-08-25',
    dateBasis: 'commit',
    shas: ['ce905f8f'],
    basis:
      "KANE'S CONFIRMATION, ASKED FOR AND RECORDED: asked in the 2026-08-26 board-sync review which Pending Deploy rows he had actually looked at in production, he answered 'All of them - I've tested everything'. That answer is the evidence for Done on this row, and it is written here rather than assumed, because a blanket confirmation that is not recorded is indistinguishable from a guess six weeks later. It was NOT applied blanket: the Tickets row is held at Pending Deploy in this same pass, because an assertion cannot import an n8n workflow and the two webhook slugs are measurably absent. IN PROGRESS -> PENDING DEPLOY: `ce905f8f` IS now an ancestor of `origin/main` (pushed 2026-08-26, origin/main 667dfe9d -> 854a78b8), re-verified with `git merge-base --is-ancestor` after a fetch rather than read off the push output. The staged wording said IN PROGRESS BECAUSE OF GIT, NOT BECAUSE OF DOUBT, which is exactly why it moves the moment git changes its answer. THE WORK: the 2026-08-24 rebrand pointed all three processor registries at `/kolan.png` and never added the asset, so Kolan - the highest-volume rail - rendered the fallback orange monogram on every screen for a day with nothing erroring. ProcessorLogo's onError fallback is what made that survivable and also what made it invisible: a referenced-but-absent logo looks exactly like a card that was never given one. The real mark is installed at `/kolan.svg`, cropped from the official lockup via viewBox so no path was re-drawn, mark-only on purpose because that lockup's wordmark is WHITE against a bg-white plate. DELIBERATELY ITS OWN ROW rather than folded into the Done Kolan-rename row: that row claims shipped-and-proven, and this is the surface it shipped broken. Scored 2 and not 1 because `logoSrc` had six consumers, one carrying a `mix-blend-multiply` banned by ui-standards 6.4 that flattened the new mark to a featureless black square in light mode only - disguised as a theme bug by its correctly-rendering `dark:mix-blend-normal` companion - and because it added `processor-logo-assets.test.ts`, whose three assertions were confirmed FAILING against the broken states before being left green. NOT DONE: nobody has looked at a Kolan card in production since the deploy.",
  },
  {
    name: 'Board sync closed twelve undeclared features and refused one closure a measurement disproved',
    status: 'Done',
    completed: '2026-08-25',
    dateBasis: 'commit',
    shas: ['6768db36'],
    basis:
      "KANE'S CONFIRMATION, ASKED FOR AND RECORDED: asked in the 2026-08-26 board-sync review which Pending Deploy rows he had actually looked at in production, he answered 'All of them - I've tested everything'. That answer is the evidence for Done on this row, and it is written here rather than assumed, because a blanket confirmation that is not recorded is indistinguishable from a guess six weeks later. It was NOT applied blanket: the Tickets row is held at Pending Deploy in this same pass, because an assertion cannot import an n8n workflow and the two webhook slugs are measurably absent. IN PROGRESS -> PENDING DEPLOY on the push (`6768db36` is now an ancestor of `origin/main`). The staged row said the artefact and the commit had different states - 14 board rows already written and verified, the code that produced them unpushed - and that gap is now closed from the git side. THE WORK: asked to update the board for anything fixed and make sure completion dates were set, it MEASURED before writing - `verify.mts` returned 0 Done rows without a Completed Date across all 188 of our rows, so the date half of the ask was already true - and the real gap turned out to be that twelve shipped features had no board row at all. 14 rows created, 12 Done, 40 SP; the reconciler patched 37 epics and 202 tasks; `verify.mts` re-read the board afterwards, PASS 202/202. Clustered on FILE OVERLAP, never on subject lines: `7b9fe312` is titled ATTESTATION and contains no attestation code, `681662f7` is the commit that actually changes Attestation, and `667dfe9d` titled 'Fix' is the sheet_synced repair. TWO CLOSURES REFUSED on evidence rather than on a blanket 'All of those are deployed already Ive tested them'. BOTH REFUSALS HAVE NOW BEEN SETTLED BY THE FACTS THEY WERE WAITING ON, which is the argument for having refused rather than a footnote to it: `1f94ff70` was pushed today and its row moves in this same pass, and the Kolan `payout_brand` migration was probed read-only instead of asserted. TOOLING: `TaskPriority` modelled two labels while the board carries four, so every row below High was silently unlabelled - extended to Critical/High/Medium/Low, an ADDITION to what the reconciler can write, not a loosened guard. NOT DONE: this pass's own writes are the only thing that would confirm it, and confirming yourself is not evidence.",
  },
  {
    name: 'Two documentation passes, and the two indexes that were silently dropping entries',
    status: 'Done',
    completed: '2026-08-26',
    dateBasis: 'commit',
    shas: ['59719213', '7bbf99c2'],
    basis:
      "KANE'S CONFIRMATION, ASKED FOR AND RECORDED: asked in the 2026-08-26 board-sync review which Pending Deploy rows he had actually looked at in production, he answered 'All of them - I've tested everything'. That answer is the evidence for Done on this row, and it is written here rather than assumed, because a blanket confirmation that is not recorded is indistinguishable from a guess six weeks later. It was NOT applied blanket: the Tickets row is held at Pending Deploy in this same pass, because an assertion cannot import an n8n workflow and the two webhook slugs are measurably absent. IN PROGRESS -> PENDING DEPLOY: both `59719213` and `7bbf99c2` are now ancestors of `origin/main`, so these documents are readable by someone other than this checkout - which was the entire reason the row sat open. Two passes collapsed into ONE row on file overlap: both are session logs plus the documentation gaps that reading them exposed. PASS 1 (`59719213`, 08-25): the 15-session log, plus the two shipped-but-undocumented surfaces the reading surfaced - the Payroll Wizard step-rail load progress line, whose one load-bearing invariant is that prediction alone never reaches 100%, and the pay-structure natural-key upsert fix (714 structures affected) together with the still-open COP-written-as-PHP gap. PASS 2 (`7bbf99c2`, 08-26) logged what pass 1 missed and CORRECTED an entry it got wrong: it had recorded a concurrent session as having ended with nothing shipped, when that session was eleven minutes from committing 40 SP of board work. It then closed two index defects, both silent and both self-concealing. `MEMORY.md` is loaded into every session and TRUNCATES past roughly 24.4KB with nothing saying so; it had reached 26,272 bytes, so every entry below the cut was invisible to every session that started while it was oversized - a memory that was written and is not loaded looks exactly like one that was never written. 47 index hooks rewritten to their one discriminating fact, all 193 entries kept, no slug changed, 0 broken links and 0 orphan files verified in BOTH directions. And `docs/README.md` had drifted 22 feature docs behind `docs/features/`, including `monday-board-sync.md`, which shipped the day before with its INDEX row and no README row. Not a feature, and logged anyway: the governing docs are what both skills read first, so an index that drops entries is a gap in every later decision. NOT DONE: documentation is proven by someone reading it and finding what they needed, and nobody has yet.",
  },
  {
    ...
    shas: ['1f94ff70'],
    basis:
      "KANE'S CONFIRMATION, ASKED FOR AND RECORDED: asked in the 2026-08-26 board-sync review which Pending Deploy rows he had actually looked at in production, he answered 'All of them - I've tested everything'. That answer is the evidence for Done on this row, and it is written here rather than assumed, because a blanket confirmation that is not recorded is indistinguishable from a guess six weeks later. It was NOT applied blanket: the Tickets row is held at Pending Deploy in this same pass, because an assertion cannot import an n8n workflow and the two webhook slugs are measurably absent. THE REFUSAL FROM THE LAST PASS IS NOW RESOLVED, AND RESOLVED THE ONLY WAY IT COULD BE. On 2026-08-25 Kane said 'All of those are deployed already Ive tested them' and this row was still held at In Progress, because `1f94ff70` was not an ancestor of `origin/main` - re-fetched twice to be sure. A confirmation cannot push a commit. It was pushed 2026-08-26 in the 9-commit run 667dfe9d..854a78b8, `git merge-base --is-ancestor 1f94ff70 origin/main` now passes, and the row advances on THAT fact rather than on the earlier assertion. This is the case FOR the honesty gate rather than an argument against it: the row was wrong for one day, and would have been wrong indefinitely had it been closed on the confirmation, because the code genuinely was not in production when the confirmation was given. NOT DONE: being on origin/main means Vercel will deploy it, not that anyone has opened a dispatch export and seen the three columns populated.",
  },
  {
    ...
    shas: ['854a78b8'],
    basis:
      "KANE'S CONFIRMATION, ASKED FOR AND RECORDED: asked in the 2026-08-26 board-sync review which Pending Deploy rows he had actually looked at in production, he answered 'All of them - I've tested everything'. That answer is the evidence for Done on this row, and it is written here rather than assumed, because a blanket confirmation that is not recorded is indistinguishable from a guess six weeks later. It was NOT applied blanket: the Tickets row is held at Pending Deploy in this same pass, because an assertion cannot import an n8n workflow and the two webhook slugs are measurably absent. READY TO START -> PENDING DEPLOY. Fixed today via the `hardening` skill, as the memory entry insisted it should be rather than an inline patch. THE BUG: `probeTable()` used `select('*', {head:true})` and read a falsy error as APPLIED, but PostgREST returns NO error at all and `count:null` for a table that does not exist, so every table-creating migration that never ran was reported APPLIED; it hid because `head:true` works perfectly for tables that DO exist. `probeColumn()` failed differently - a missing column errors with `code:undefined` and an EMPTY message, so it fell through to the catch-all and landed INCONCLUSIVE instead of NOT APPLIED. THE FIX: existence is decided by a plain `.select(...).limit(1)` and the count is a SECOND query, so `head:true` can never be what decides whether a migration ran; classification moved to `src/lib/db/probe-verdict.ts` with 7 tests, each confirmed FAILING against the old logic first by transcribing it verbatim and running it, rather than assumed to be new. TWO MORE SITES OF THE SAME CLASS turned up while fixing it, neither previously noticed, both reading a null count as a legitimate zero: the MESA opt-in cleanup would have reported APPLIED 'zero opt_in rows remain' for a table that might not exist, and the `active_employees` definer probe would have blamed `security_invoker` for a count that never resolved. Both now branch on null via `readCount()`; there is no `?? 0` left in the verdict path. THE ACTUAL GUARD is the NEGATIVE CONTROL, which runs FIRST and ABORTS the whole audit unless a table and a column that cannot exist are both reported missing - a probe that cannot detect absence produces a report indistinguishable from one that found none, which is precisely how this survived from 2026-08-20, and a check that runs every time is worth more than an assertion that the fix works. Post-fix production run: APPLIED 25 / NOT APPLIED 0 / INCONCLUSIVE 0, control correctly MISSING both ways. Scored 3 as planned and left at 3.",
  },
  {
    name: 'Orientation gets its own tab on the HR New Hire Checklist — listed vs attended, per week',
    status: 'Done',
    completed: '2026-08-26',
    dateBasis: 'commit',
    shas: ['72f41e44'],
    basis:
      "KANE'S CONFIRMATION, ASKED FOR AND RECORDED: asked in the 2026-08-26 board-sync review which Pending Deploy rows he had actually looked at in production, he answered 'All of them - I've tested everything'. That answer is the evidence for Done on this row, and it is written here rather than assumed, because a blanket confirmation that is not recorded is indistinguishable from a guess six weeks later. It was NOT applied blanket: the Tickets row is held at Pending Deploy in this same pass, because an assertion cannot import an n8n workflow and the two webhook slugs are measurably absent. UNDECLARED UNTIL THIS PASS, and undeclared for a mechanical reason worth naming: pass 14 was STAGED at 09:18 with `AUDIT_RANGE = 1f94ff70..HEAD` and `AUDIT_COMMITS = 4`, and this landed at 09:27 - nine minutes after that snapshot. Nothing re-reads a staged range, which is the same defect that left twelve features undeclared before it. THE WORK: the HR half of orientation attendance, peer to the 5-SP manager tab rather than an echo of it - 13 files, 1,820 insertions, a new route (`app/api/hr/orientation-attendance/route.ts`), a new tested module (`orientation-week-stats.ts`, 236 lines with 337 of test) and a 570-line panel. The load-bearing distinction it encodes is that LISTED IS NOT STAGED: the roster names who was expected, the attendance rate is computed against who was actually STAGED, and a week where nothing was marked renders as a NOTE rather than 0% - a 0% that actually means 'nobody recorded it' is a lie about attendance, and it would be read as one. Week buckets follow the HR `period_start` rather than the manager tab's own week, and one cached fetch serves every week. Scored 5, the same as the manager tab: a new route, a new tested module and a new inner tab on a live HR screen. Not 8 - no rate, no dispatch row, no score component moves. PENDING DEPLOY: `72f41e44` is an ancestor of `origin/main` as of today's push, and nobody has opened the tab in production.",
  },
  {
    name: 'Dispatch exports carried 833 pending rows with no payout destination',
    status: 'Done',
    completed: '2026-08-26',
    dateBasis: 'commit',
    shas: ['0e7dc633'],
    basis:
      "KANE'S CONFIRMATION, ASKED FOR AND RECORDED: asked in the 2026-08-26 board-sync review which Pending Deploy rows he had actually looked at in production, he answered 'All of them - I've tested everything'. That answer is the evidence for Done on this row, and it is written here rather than assumed, because a blanket confirmation that is not recorded is indistinguishable from a guess six weeks later. It was NOT applied blanket: the Tickets row is held at Pending Deploy in this same pass, because an assertion cannot import an n8n workflow and the two webhook slugs are measurably absent. UNDECLARED UNTIL THIS PASS - it landed at 12:44 today, more than three hours after pass 14 was staged. A SEPARATE ROW from the Adjustment/COP/System Bonus export row that shares this pass, deliberately: that row was about columns present and empty, this is about 833 rows carrying no payout destination at all. They are different failures of the same file, found on different days, and merging them would make one of the two invisible. The exporter's `detailFields` covered some rails and silently omitted Kolan and HiGlobe, so a clerk received a file naming WHO to pay and not WHERE - the worst shape an export can take, because a missing destination looks exactly like a complete file until someone tries to pay from it. The fix is 35 lines; the guard is the point, and `detailFields` coverage is now a TEST (81 lines), so a rail added without a destination fails at build rather than at the bank. Scored 2: no pricing, no new surface, one exporter - but not 1, because it is a money-adjacent omission on a live Accounting artefact. PENDING DEPLOY: on `origin/main`, not confirmed live.",
  },
  {
    name: 'Exported pay stubs name the CURRENT department, paid or not',
    status: 'Done',
    completed: '2026-08-26',
    dateBasis: 'commit',
    shas: ['bdacb862'],
    basis:
      "KANE'S CONFIRMATION, ASKED FOR AND RECORDED: asked in the 2026-08-26 board-sync review which Pending Deploy rows he had actually looked at in production, he answered 'All of them - I've tested everything'. That answer is the evidence for Done on this row, and it is written here rather than assumed, because a blanket confirmation that is not recorded is indistinguishable from a guess six weeks later. It was NOT applied blanket: the Tickets row is held at Pending Deploy in this same pass, because an assertion cannot import an n8n workflow and the two webhook slugs are measurably absent. UNDECLARED UNTIL THIS PASS, and it landed DURING this pass - committed by a CONCURRENT SESSION in the same checkout at 13:41, after the audit range had already been re-derived once. Declared here rather than left for that session because an undeclared shipped feature is the exact gap the previous pass found twelve of; the byte-exact plan name is what stops a duplicate row if that session files it too. THE WORK: exported pay stubs head themselves with the department the roster holds TODAY, marked `(current)`, on every week the export covers - paid, staged or reconstructed. It does NOT contradict 'a paid stub is frozen', and the doc argues the distinction rather than asserting it: the money and the per-week record are untouched, a person's department is a fact about THEM and not about that week, and the in-week moves keep being explained by the per-week Department Change column. ONE server-side resolution (`getEmployeeMasterRecord`, active-rows-only, ordered by `last_seen_upload_id` so a duplicated identity resolves to the newest row) feeds all three export call sites, replacing `PayStubModal`'s use of the week's own frozen department - the one genuinely stale header. The fallback is a STATE, not a mask: a null current department is REAL (an off-boarded person has no active master row), so it falls back to the newest week's own department and deliberately omits `(current)`. Both sides go through `formatDeptLabel`, so a raw `hsl:*` key can never reach a header; a test pins that. Scored 3: one pure tested function and three call sites, no money path - not 5, and not 2 given 164 lines of test and an explicit ruling about what freezing does and does not cover. PENDING DEPLOY: on `origin/main`, not confirmed live.",
  },
  {
    ...
    basis:
      "STATUS UNCHANGED, BASIS CORRECTED ON A MEASUREMENT - and the correction runs in BOTH directions, which is why it is worth writing down. This row was held at Pending Deploy on TWO blockers recorded 2026-08-21. Both were measured today rather than re-asserted, using the migration probe fixed in this same pass, and they came back SPLIT. BLOCKER 1 IS CLOSED, AND WAS ALREADY CLOSED WHEN IT WAS WRITTEN DOWN: `ticket.moved` IS present in the live `employee_notifications_type_check` (42 permitted types), read off `pg_get_constraintdef` with a negative control (`definitely.not.a.type.xyz`) correctly rejected, so the read can detect absence rather than merely fail to notice it. A parity sweep of every notification type the app can insert against that constraint found ZERO rejected - that sweep is the check that would have caught `kpi.scored` shipping dead for three days behind a `console.warn`. BLOCKER 2 IS GENUINELY OPEN, AND NOW MEASURED RATHER THAN CLAIMED: `webhooks.config` in `app_settings` holds 22 entries, `ticket_created` / `ticket_done` / `ticket_assigned` are configured and active, and `ticket_replied` and `ticket_moved` are BOTH ABSENT, so `resolveWebhookUrl` returns null and both hooks no-op silently by design. THE FEATURE IS THEREFORE HALF-LIVE: the in-app bell rings and the email does not - a state neither the doc nor the memory entry described, because both asserted the whole thing was dead. `tickets-board.md` and the `tickets-update-notifications` memory entry were corrected in `854a78b8` to say which half is which. RECIPIENT RULES unchanged as Kane set them: a status move emails the TICKET CREATOR only, a comment emails the COUNTERPARTY, any move is emailed including a backward bounce, and `done` still rides notifyTicketDone.",
    blockers: [
      'The two n8n workflows are still not imported - MEASURED, not assumed: ticket_replied and ticket_moved are absent from webhooks.config (22 entries, the other three ticket slugs active). The email leg stays inert until both workflows are imported and each URL pasted into Admin -> Webhooks.',
    ],
  },
  {
    ...
    shas: ['31b11050'],
    basis:
      "KANE'S CONFIRMATION, ASKED FOR AND RECORDED: asked in the 2026-08-26 board-sync review which Pending Deploy rows he had actually looked at in production, he answered 'All of them - I've tested everything'. That answer is the evidence for Done on this row, and it is written here rather than assumed, because a blanket confirmation that is not recorded is indistinguishable from a guess six weeks later. It was NOT applied blanket: the Tickets row is held at Pending Deploy in this same pass, because an assertion cannot import an n8n workflow and the two webhook slugs are measurably absent. AND THE ROW'S OWN BLOCKER IS NOW MEASURABLY FALSE, WHICH MATTERS MORE THAN THE CONFIRMATION. It was held on 'No kpi.scored notification has ever been delivered' - measured 2026-08-20 as ZERO rows against 3,694 payroll.available, the feature having shipped dead for three days because the type CHECK rejected every insert and all four call sites swallow a failed notify into console.warn. Re-measured 2026-08-26: employee_notifications holds 702 kpi.scored rows, most recent 2026-08-25T00:11Z, with a negative control (definitely.not.a.type.xyz) returning 0 in the same run so the count can distinguish absence from a failed query. The feature has demonstrably been delivered 702 times. Completed Date is 31b11050's commit date, 2026-08-17, which sits inside S26's Aug 4-17 attribution (S26 absorbs the Aug 16-17 gap days) - the work finished when it was committed; the delivery is what unblocked Done, not what re-dated it. Two follow-up rows remain open against it and are tracked separately: the months-old-week floor, and making swallowed notify failures observable.",
  },
  {
    ...
    basis:
      "KANE'S CONFIRMATION, ASKED FOR AND RECORDED: asked in the 2026-08-26 board-sync review which Pending Deploy rows he had actually looked at in production, he answered 'All of them - I've tested everything'. That answer is the evidence for Done on this row, and it is written here rather than assumed, because a blanket confirmation that is not recorded is indistinguishable from a guess six weeks later. It was NOT applied blanket: the Tickets row is held at Pending Deploy in this same pass, because an assertion cannot import an n8n workflow and the two webhook slugs are measurably absent. AND ITS SECOND BLOCKER IS CLOSED ON EVIDENCE, NOT ON THE CONFIRMATION. The row carried 'the first real payroll.hours_gap insert is unproven until the next Hubstaff ingest runs; the DDL is applied but the write path has never fired' - which no assertion could settle, because a confirmation cannot make an insert that never happened have happened. Measured 2026-08-26: employee_notifications holds 20 payroll.hours_gap rows, so the write path HAS fired, with a negative control returning 0 in the same run. THE WORK: nobody had offboarded jvincec@ and nothing said so - he sat Active with zero hours from 2026-08-05. The DETECTOR already existed; what was missing was DELIVERY. classifyZeroHours (245 lines, 221 of test) is the ONE rule now shared by the Overview tile, a new Readiness No Hours tab, and a payroll.hours_gap notification fired on ingest to accounting role holders only - never to the person with no hours. 18 files, 1,617 insertions, 44 new tests. Scored 5 because it touches NO money path, deliberately: Lead Gen stays tracked per Kane, which puts ~193 rows on the list every week, so the dimension is LISTED AND NEVER SCORED.",
  },
  {
    ...
    shas: ['7b7e9123'],
    basis:
      "KANE'S CONFIRMATION, ASKED FOR AND RECORDED: asked in the 2026-08-26 board-sync review which Pending Deploy rows he had actually looked at in production, he answered 'All of them - I've tested everything'. That answer is the evidence for Done on this row, and it is written here rather than assumed, because a blanket confirmation that is not recorded is indistinguishable from a guess six weeks later. It was NOT applied blanket: the Tickets row is held at Pending Deploy in this same pass, because an assertion cannot import an n8n workflow and the two webhook slugs are measurably absent. HUBSTAFF_EXEMPT_DEPTS matched raw master-list labels exactly, and the dept it excuses was renamed: 'Site Building' became 'Site Building (US - Freelance)' (20 people, ZERO with Hubstaff hours) and 'Site Building (PH - Freelancer)' (13, zero). The list therefore inverted its own meaning, and the Overview Hubstaff-to-Master tile and its CEO mirror reported 33 deliberately-untracked freelancers as unexplained reconciliation gaps. The untouched 'SMM Freelancer' label kept working, which is how it stayed invisible. Fixed by retrying the match once with a trailing parenthetical qualifier stripped; the test pins the negative control (a dept whose base label was never exempt stays tracked however it is qualified) and pins Lead Gen NOT exempt per Kane. Filed as its own 2-SP row rather than folded into the zero-hours feature because it was wrong on its own, on a surface that already shipped, before any of that was added - a PREREQUISITE, not a part. It shares sha 7b7e9123 with that feature, which is why the two rows cite the same commit.",
  },
  {
    ...
    basis:
      "KANE'S CONFIRMATION, ASKED FOR AND RECORDED: asked in the 2026-08-26 board-sync review which Pending Deploy rows he had actually looked at in production, he answered 'All of them - I've tested everything'. That answer is the evidence for Done on this row, and it is written here rather than assumed, because a blanket confirmation that is not recorded is indistinguishable from a guess six weeks later. It was NOT applied blanket: the Tickets row is held at Pending Deploy in this same pass, because an assertion cannot import an n8n workflow and the two webhook slugs are measurably absent. THREE commits, ONE row: all three rewrite the same tested module (dept-rail.ts) and the same screen (BonusCatalog.tsx), so clustering on FILE OVERLAP makes them one feature built and then hardened the same day, not three. 24d6d0a1 built it - dept-rail.ts (264 lines) with 269 of test, a 598-line screen change, and the rule that the rail is a TREE: a Pay Structure renders under the member PLACEMENT rather than whatever departmentKey it stores, and the bare parent claims NOBODY, which is why the HSL sub-teams nest under a retractable Hogan Smith Law instead of flattening into it. 6cb643b2 then resolved a structure owner by IDENTITY and not by email alone - Baldonebro was being held by the parent because the email matched first. 47e84590 closed an adder guard that could overwrite a live rate from a blank form, plus five more review findings. Scored 5 as the peer of the 5-SP People Bank-changes band row: a new tested module plus one screen, no money path, no new table.",
  },
  {
    ...
    shas: ['1937731f'],
    basis:
      "KANE'S CONFIRMATION, ASKED FOR AND RECORDED: asked in the 2026-08-26 board-sync review which Pending Deploy rows he had actually looked at in production, he answered 'All of them - I've tested everything'. That answer is the evidence for Done on this row, and it is written here rather than assumed, because a blanket confirmation that is not recorded is indistinguishable from a guess six weeks later. It was NOT applied blanket: the Tickets row is held at Pending Deploy in this same pass, because an assertion cannot import an n8n workflow and the two webhook slugs are measurably absent. The Payment Catalog was still listing people who had left. 14 files bring four new tested modules - catalog-roster-visibility.ts (130 lines, 235 of test), offboard-evidence.ts (160), master-date.ts (48, 37 of test) and catalog-offboarded-emails.ts (114) - plus a verify script. Every one of the four guards deliberately fails toward KEEPING a person, because dropping someone still employed is the worse error; evidence is read on WORK emails ONLY, since a shared personal inbox imports someone else's departure. Measured at the time: active_employees carried 0 stamped and 294 gone. 110 lines came OUT of payroll-readiness.ts as the rule was centralised, so this is a consolidation as much as an addition. Not 8: no money path, no new table, no dispatch row. Not 3: four new modules, 272 lines of test, and a behavioural change to who appears on a live Accounting screen.",
  },
  {
    ...
    basis:
      "KANE'S CONFIRMATION, ASKED FOR AND RECORDED: asked in the 2026-08-26 board-sync review which Pending Deploy rows he had actually looked at in production, he answered 'All of them - I've tested everything'. That answer is the evidence for Done on this row, and it is written here rather than assumed, because a blanket confirmation that is not recorded is indistinguishable from a guess six weeks later. It was NOT applied blanket: the Tickets row is held at Pending Deploy in this same pass, because an assertion cannot import an n8n workflow and the two webhook slugs are measurably absent. TWO commits, ONE row: de0fa485 only re-animates ValidationFullScreen.tsx, the component 56390cb9 created, so it is the same feature finishing rather than a second one. A named human vouches for ONE person's pay and Lenny sees that vouch at Mark Paid: a new route (181 lines), a tested module (197 plus 198 of test), a hook, the full-screen overlay and the Mark Paid surfacing - 13 files, ~1,750 lines including the 212-line feature doc. SCORED 5 AND NOT 8 despite the line count, because it RECORDS a human judgement and never prices anything: no rate, no amount and no score component moves. The one genuinely novel part is that the validation CANNOT live on payment_dispatches - at step 7 no dispatch row exists yet - so it rides an app_settings blob written compare-and-swap, and Mark Paid keys it on the WORK email, never the payout address, which is shared and recycled in the master list.",
  },
  {
    ...
    shas: ['ae947bbc'],
    basis:
      "KANE'S CONFIRMATION, ASKED FOR AND RECORDED: asked in the 2026-08-26 board-sync review which Pending Deploy rows he had actually looked at in production, he answered 'All of them - I've tested everything'. That answer is the evidence for Done on this row, and it is written here rather than assumed, because a blanket confirmation that is not recorded is indistinguishable from a guess six weeks later. It was NOT applied blanket: the Tickets row is held at Pending Deploy in this same pass, because an assertion cannot import an n8n workflow and the two webhook slugs are measurably absent. Deliberately a SECOND row rather than an edit of the Done 2-SP row about orphanage OT pricing at the full 1.5x rate. That row was a seventeen-line price correction; this is the hardening that followed it - 6 files, 1,763 insertions: pricing extracted into orphanage-pay-pricing.ts as the one rule shared by the paste tool and the Re-price action, below-regular OT REFUSED outright rather than silently accepted (below-regular is the shape of a differential, and quietly deriving would hide bad rate data), an audit script for the divergence, and the 2026-08-09 week repaired. It does not REVERSE the earlier row - it makes the same rule unfalsifiable - so both rows stand. STILL OPEN AND EXPLICITLY NOT CLOSED BY THIS ROW, tracked separately rather than carried as a blocker on it: erict@ 5,373 pesos remains invisible, 9 people are still underpaid 27,550.20 on the 2026-08-09 week, and the additions blob is still last-writer-wins on the whole object. Those are their own work, not this row's remainder.",
  },
];
