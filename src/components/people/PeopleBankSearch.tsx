...
export default function PeopleBankSearch<T extends BankSearchPerson>({
  ...
  const fade = { duration: reduceMotion ? 0 : 0.2, ease: EASE };
        ...
        {selected ? (
          ...
          >
            {/* Same landing as Payment Catalog → Search: the logo over a vertically
                centred bar when idle, both moving to the top once a query is typed.
                Only the top spacer animates (a pair both easing to zero keeps
                their ratio, freezing the hero until the end), and the navy logo
                renders as a white silhouette in dark mode. */}
            <div
              aria-hidden
              className={cn(
                'shrink-0 transition-[flex-grow] duration-500 ease-out motion-reduce:transition-none',
                typed ? 'grow-0' : 'grow-[0.85]',
              )}
            />
            <div className="mx-auto w-full max-w-xl">
              <div
                className={cn(
                  'flex justify-center transition-all duration-300 motion-reduce:transition-none',
                  typed ? 'mb-4' : 'mb-7',
                )}
              >
                <img
                  ...
                  draggable={false}
                  className={cn(
                    'select-none object-contain transition-all duration-300 motion-reduce:transition-none dark:brightness-0 dark:invert',
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={cn(
                    'w-full rounded-full border border-zinc-200 bg-white py-3.5 pl-12 pr-10 text-sm text-zinc-800 shadow-sm outline-none transition-shadow placeholder:text-zinc-400 hover:shadow-md focus:shadow-md dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-200',
                    accent.ring,
                  )}
                />
                {query && (
              ...
              {/* Fixed-height slot: the hint fades instead of unmounting, so nothing
                  below it jumps while the bar moves. */}
              <div className="mt-3 min-h-8">
                <p
                  aria-hidden={typed}
                  className={cn(
                    'text-center text-xs text-zinc-400 transition-opacity duration-300 motion-reduce:transition-none dark:text-zinc-500',
            ...
            {typed &&
              (error && rows.length === 0 ? (
                <p className="mt-2 rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-xs text-rose-700 dark:border-rose-900/50 dark:bg-rose-950/30 dark:text-rose-300">
                  The roster could not be loaded, so there is nothing to search: {error}
                </p>
              ) : loading && rows.length === 0 ? (
                <div className="mt-2 flex items-center justify-center gap-2 py-8 text-sm text-zinc-500">
                  <Loader2 className="h-4 w-4 animate-spin motion-reduce:animate-none" /> Loading the roster…
                </div>
              ) : results.length === 0 ? (
                <div className="mt-2 rounded-xl border border-dashed border-zinc-300 px-4 py-8 text-center dark:border-zinc-700">
                  ...
                  <div className="space-y-2">
                    {pageRows.map((r, i) => (
                      <ResultRow
                        ...
                        person={r}
                        onView={() => setSelectedKey(personKey(r))}
                      />
                    ))}
                  </div>
  ...
  return (
    <div>
      <button
        type="button"
        onClick={onBack}
        className="mb-3 inline-flex items-center gap-1.5 rounded-md px-1.5 py-1 text-[13px] font-medium text-zinc-500 transition-colors hover:text-zinc-800 dark:text-zinc-400 dark:hover:text-zinc-200"
      >
