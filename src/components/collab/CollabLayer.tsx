 ...
 *
 * Dashboard-agnostic engine: a host (Accounting, HR, …) mounts it with its own
 * Realtime channel names, section-label map, and accent via thin wrappers
 * (`AccountingCollabLayer`, `HrCollabLayer`). Each dashboard is its own
 * collaboration room — the channel names keep the rosters separate.
 *
 ...
 *     page. Shows every other user currently online in the same dashboard with
 ...
 *     cursors from peers who are on the SAME section. So you only ever see a
 *     peer's cursor when you are both on, e.g., Overview.
 *
 * On top of the passive layer there is an opt-in "Observe" mode: clicking
 * Observe on a peer in the avatar rail mirrors THAT peer's live screen into a
 * full-screen overlay in real time (rrweb), the wizard's driver-mode analogue.
 * A banner with a "Stop observing" opt-out is shown while active.
 *
 ...
 * NOT section-scoped — you can nudge anyone in the dashboard from any tab — and
...
// Fallback channel names + section labels — used when a host dashboard mounts
// this layer without overriding them. Defaults match the original Accounting
// dashboard so its existing wrapper keeps working with zero config.
const DEFAULT_CHANNEL = 'accounting-collab';
const DEFAULT_COBROWSE_CHANNEL = 'accounting-cobrowse';
...
function sectionLabel(section: string, labels: Record<string, string>): string {
  return labels[section] ?? section;
}

/**
 * Visual accent for the Observe affordances + the "observing" avatar ring.
 * Tailwind class fragments must be LITERAL strings in the host wrapper files so
 * the JIT compiler picks them up (e.g. `bg-emerald-500`, `bg-orange-500`).
 */
export interface CollabAccent {
  /** Idle "Observe" button background (+ hover). */
  observeBtn: string;
  /** Idle eye-badge background (+ text color). */
  observeBadge: string;
  /** focus-visible ring color class. */
  focusRing: string;
  /** Solid hex used for the bold ring around an actively-observed avatar. */
  hex: string;
  /** Glow rgba for that observing ring. */
  ringGlow: string;
  /** Glow rgba for the cobrowse surface fallback accent. */
  cobrowseGlow: string;
}

// Accounting orange — the original look, kept as the default accent.
const DEFAULT_ACCENT: CollabAccent = {
  observeBtn: 'bg-orange-500/90 hover:bg-orange-500',
  observeBadge: 'bg-orange-500 text-white',
  ...
  return (
    ...
    >
            ...
            <button
              ...
              className={
                observing
                  ...
                  : `mt-2 w-full rounded-md ${accent.observeBtn} px-2.5 py-1.5 text-[11px] font-semibold text-white shadow transition-colors`
              }
            >
      ...
      <motion.button
        ...
        className={`relative block cursor-pointer rounded-full outline-none focus-visible:ring-2 ${accent.focusRing} focus-visible:ring-offset-2 focus-visible:ring-offset-transparent`}
      >
        <div
          className="h-11 w-11 overflow-hidden rounded-full bg-white transition-shadow duration-300 dark:bg-zinc-900"
          style={{
            // While actively observing this peer, pin a bold accent ring so the
            ...
            boxShadow: observing
              ? `0 0 0 2.5px ${accent.hex}, 0 0 16px ${accent.ringGlow}, 0 4px 12px rgba(0,0,0,0.28)`
              : sameSection
      ...
      <motion.button
        ...
        transition={POP_SPRING}
        className={
          `absolute -bottom-1 -left-1 flex h-5 w-5 items-center justify-center rounded-full border-2 border-white shadow-md outline-none focus-visible:ring-2 ${accent.focusRing} dark:border-zinc-900 ` +
          (observing ? 'bg-rose-500 text-white' : accent.observeBadge)
        }
      >
...
export interface CollabLayerProps {
  ...
  /** Realtime channel for presence + cursors + pings. Defaults to Accounting. */
  channel?: string;
  /** Realtime channel for the rrweb Observe stream. Defaults to Accounting. */
  cobrowseChannel?: string;
  /** Section id -> human label, for tooltips. Defaults to Accounting's tabs. */
  sectionLabels?: Record<string, string>;
  /** Visual accent for Observe affordances. Defaults to Accounting orange. */
  accent?: CollabAccent;
...
export default function CollabLayer({
  ...
  cobrowseChannel = DEFAULT_COBROWSE_CHANNEL,
  ...
  // Co-browse runs for every user in this dashboard: the DRIVER half (record +
  // stream when watched) must always be live so this person is observable even
  // when they aren't observing anyone. The OBSERVER half activates when
  // observedEmail is set. The channel is dashboard-scoped so HR observers only
  // ever stream/receive HR screens (and vice-versa for Accounting).
  const { setReplayContainer, status: cobrowseStatus } = useCobrowse({
    selfEmail: normSelf,
    observedEmail,
    channel: cobrowseChannel,
  });
      ...
      <AnimatePresence>
        {observedEmail && (
          <CobrowseSurface
            ...
            accent={observedColor ?? { bg: accent.hex, glow: accent.cobrowseGlow }}
