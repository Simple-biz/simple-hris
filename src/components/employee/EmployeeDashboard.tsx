...
export default function EmployeeDashboard({ employeeEmail, needsPhoto = false, needsBank = false, needsSkillSet = false, onNavigateToProfile, onNavigateToNotifications, unreadNotifications = 0 }: EmployeeDashboardProps) {
  ...
  const isTechBonusWeekPast = useMemo(() => {
    ...
    if (refMonday.getTime() < eligibleFrom.getTime()) return false;
    // The month's tech PAY-PERIOD Monday: the System Bonus modal pick when the
    // wizard saved one, else the 3rd-week heuristic's pay Monday (week3Mon − 7,
    // salary Tuesday landing in week 3). "Past" pivot = that pay Monday + 7 —
    // for the heuristic this is exactly the old week3Mon comparison.
  ...
  }, [isTechnologyBonusActive, employeeStartDate, selectedFileWeek, pabMonthRange, techWeekOverrides]);
  ...
  const techBonusSalaryThisWeek = useMemo<Date | null>(() => {
    ...
    // Override-aware shared gate: a wizard System Bonus payout-week pick for the
    // month wins; otherwise the legacy 3rd-week rule.
    return resolveIsTechBonusWeek(payPeriodMon, techWeekOverrides) ? salaryDate : null;
  }, [employeeStartDate, techWeekOverrides]);
  ...
  return (
    <div className="box-border flex h-full min-h-0 flex-col gap-2 overflow-y-auto overscroll-y-contain bg-gradient-to-br from-white via-orange-50/30 to-blue-50/20 px-3 py-2 [scrollbar-gutter:stable] [@media(max-height:900px)]:gap-1.5 sm:px-4 sm:py-3 md:px-5 lg:gap-3 lg:py-3 dark:bg-none dark:bg-[#0d1117]">
      ...
      <Dialog open={mobileHelpOpen} onOpenChange={setMobileHelpOpen}>
        <DialogContent
          className="max-h-[min(90vh,760px)] w-[calc(100vw-1.25rem)] max-w-3xl gap-0 overflow-y-auto border-orange-100/70 bg-gradient-to-br from-white via-orange-50/35 to-blue-50/25 p-0 sm:max-w-3xl dark:border-blue-950/50 dark:from-[#0d1117] dark:via-[#0f1729] dark:to-[#0a1628]"
          showCloseButton
        >
