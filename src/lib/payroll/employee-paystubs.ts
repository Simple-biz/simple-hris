// Employee pay-stub statements — the ONE assembly behind every self-serve
// reader of a person's weekly statements.
//
// Moved here verbatim from `app/api/employee/paystub/route.ts` (2026-09-10) so
// the Certificate of Engagement can quote what a worker's recent statements
// say without a second copy of the recovery tiers. The route is now a thin HTTP
// shell over these functions; `listEmployeePayStubs` is the all-weeks mode it
// used to inline. Nothing in here changed behaviour in the move — the
// statement arithmetic (`buildView`, `reconstructStubForWeek`) is the same
// code, and `docs/features/paystub-dispatch.md` § Recovered-week snapshots
// still governs it.
//
// Every function is scoped to ONE person by the emails the caller resolved
// from a session or an authorised lookup — never a query parameter.
 ...
 *  source_files → empty. */
export async function listUploadIndex(): Promise<{
  files: string[];
...
export function emptyWeekDiscretionary(): WeekDiscretionary {
  return {
    finalPay: null,
    fxRate: null,
    adjustment: 0,
...
export async function currentFxRate(): Promise<number> {
  try {
 ...
 */
export async function reconstructStubForWeek(params: {
  sourceFile: string;
  emails: string[];
  name: string;
  department: string;
 ...
 */
export async function copDecoratorForEmails(
...
export function paidAtByFileFrom(
 ...
 * the single-week modal: paid → the frozen as-paid payload; unpaid → the staged
 * payload with any newer wizard-snapshot figures merged over it, so the row
 * total, the modal, and the PDF/XLSX export all agree.
 *
 * Employee-scope catalog rate claims gate that merge — a stale wizard session's
 * snapshot must not overwrite corrected staged figures (see
 * mergeSnapshotIntoStaged). The caller reads them ONCE and shares them across
 * every week's row.
 */
export function freshStagedViewWith(catalogClaims: Map<string, CatalogRateClaim>) {
  return (
    p: {
...
/**
 * Every week's FULL statement for one person — the all-weeks mode the Pay Stubs
 * PDF/XLSX export and the Certificate of Engagement both read. One row per pay
 * week (paid > staged > newest upload), newest first, COP equivalents stamped
 * for COP-country payees.
 *
 * `email` is the person's session or authorised work email; their other
 * addresses are folded in from the master record (`callerEmails`).
 *
 * `sinceWeekEnd` (YYYY-MM-DD) narrows the candidate set to weeks ENDING on or
 * after that date BEFORE any recovery work runs, so a reader that only needs
 * the recent weeks does not pay for the whole archive. A file whose week
 * cannot be dated from its name is dropped under a cutoff (it cannot be shown
 * to be recent); with no cutoff every file is considered, exactly as before.
 *
 * `currentDepartment` is the roster's CURRENT department (null when there is
 * no active master row) — the header the export names on every week it covers.
 */
export async function listEmployeePayStubs(
  email: string,
  opts: { sinceWeekEnd?: string | null } = {},
): Promise<{ stubs: EmployeePayStub[]; currentDepartment: string | null }> {
  const since = opts.sinceWeekEnd?.trim() || null;

  const [{ rows: dispatches }, { rows: payloads }, { files: allFilesRaw, uploadIdByFile }, catalogClaims] =
    await Promise.all([
  ...
  // The cutoff applies to the upload archive by the week each file NAMES, so a
  // week outside the window is never even a candidate for recovery.
  const allFiles = since
    ? allFilesRaw.filter((f) => {
        const range = parseDateRangeFromFilename(f);
        return range ? fmtIso(range.end) >= since : false;
      })
    : allFilesRaw;

  const stagedAll = payloads.filter(
  ...
  const officialStubs: EmployeePayStub[] = stagedAll
    ...
    // itself states (a staged row with no dated week cannot be shown recent).
    .filter((s) => (since ? !!s.view.weekEnd && s.view.weekEnd >= since : true));
  const stagedFiles = new Set(officialStubs.map((s) => s.sourceFile));
  ...
  return { stubs, currentDepartment: master?.department ?? null };
}
