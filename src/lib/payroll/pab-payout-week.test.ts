...
import { isPabPayoutWeekForRange, pabPayoutWeekForPeriodEnd } from './pab-payout-week';
...
test('pabPayoutWeekForPeriodEnd names the week the gate agrees pays', () => {
  ...
  // Aug 2–29 is carried by the file that CONTAINS Aug 29 — its own last week.
  const w = pabPayoutWeekForPeriodEnd(d(2026, 8, 29));
  assert.equal(iso(w.start), '2026-08-23');
  assert.equal(iso(w.end), '2026-08-29');
  assert.equal(isPabPayoutWeekForRange(w.start, w.end, AUG_OVERRIDE, null), true);

  // A period ending mid-week (the Mon→Fri default shape) resolves the same way.
  const s = pabPayoutWeekForPeriodEnd(d(2026, 10, 2)); // Friday
  assert.equal(iso(s.start), '2026-09-27');
  assert.equal(iso(s.end), '2026-10-03');
});
