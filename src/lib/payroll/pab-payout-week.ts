...
/**
 * The payroll FILE week that pays a period ending `periodEnd` — the week that
 * CONTAINS it, which is the same week `isPabPayoutWeekForRange` returns true
 * for. Exists so the Additions "System Bonus" strip can state the payout week
 * the way the Tech pill next to it already does.
 *
 * Disclosure only, never a gate: gates compare the week they are handed, they
 * do not construct one. Kane, 2026-09-08: the file that carries the bonus is the
 * period's last week (Aug 23–29 for Aug 2–29); the calendar week you PROCESS it
 * in is the following one, which is what "the payout week is after the period"
 * means operationally.
 */
export function pabPayoutWeekForPeriodEnd(periodEnd: Date): { start: Date; end: Date } {
  // Sun–Sat payroll weeks: back up to the Sunday on/before the period end.
  const start = new Date(
    periodEnd.getFullYear(),
    periodEnd.getMonth(),
    periodEnd.getDate() - periodEnd.getDay(),
  );
