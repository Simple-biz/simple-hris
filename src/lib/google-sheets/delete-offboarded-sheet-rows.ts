...
export type DeleteOffboardedSheetRowsResult = {
  deleted: number;
  reason?: string;
};
...
/**
 * The Offboarded tab keys on a "Personal Email" column (mirrors the detection in
 * fetch-offboarded-sheet.ts). All other columns are optional.
 */
...
/**
 * Deletes all rows in the Google Sheet "Offboarded" tab that match the given
 * personal_email or work_email. This is the tab that feeds the offboarded_sheet
 * snapshot (and thus the HR Offboarded list), so removing the row here is what
 * stops the next sync-offboarded-from-sheet cron from re-adding the person.
 *
 * Rows are deleted bottom-to-top so indices don't shift between deletes.
 * Best-effort by contract -- callers should surface the result but not block
 * other operations on failure.
 */
export async function deleteOffboardedSheetRowsByEmail(
  personalEmail: string,
  workEmail?: string,
): Promise<DeleteOffboardedSheetRowsResult> {
  ...
  if (!sheetId) {
    return { deleted: 0, reason: 'GOOGLE_SHEETS_MASTER_SHEET_ID not configured' };
  }
  ...
  if (headerIdx < 0) {
    return { deleted: 0, reason: 'header row not found in Offboarded tab' };
  }

  const headers = (values[headerIdx] ?? []).map((c) =>
    String(c ?? '').trim().toLowerCase().replace(/\s+/g, ' '),
  );
  const workCol = headers.findIndex((h) => h === 'work email' || h === 'workemail' || h === 'email');
  const personalCol = headers.findIndex(
    (h) => h === 'personal email' || h === 'personalemail' || h === 'personal_email',
  );
  ...
  if (rowsToDelete.length === 0) {
    return { deleted: 0, reason: 'not found in Offboarded tab' };
  }
