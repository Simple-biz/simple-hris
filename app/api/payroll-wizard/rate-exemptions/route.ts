...
/**
 * "Ignore" on Payroll Wizard → Readiness → No Pay Rate — the rate twin of the
 * Bank Info tab's Temporary Exemption (bank-exemptions/route.ts).
 *
 * An ignore acknowledges one person's missing pay rate for ONE pay week:
 * readiness moves them off the No Pay Rate list (and out of the score's rate
 * dimension, list and worker denominator alike) and onto the Exceptions list.
 * Week-scoped with no expiry job — they reappear next week if they log hours
 * and still have no rate.
 *
 * Readiness-only: neither the wizard's pay computation nor Payment Dispatch
 * reads this table, so an ignore changes nothing about what the person would
 * actually be paid.
 *
 * Same gates as the rest of the wizard: reading needs `view`, filing/undoing an
 * ignore needs `edit` (the same grant behind the "Set rate" / "Set bank" inline
 * fixers).
 */

/** GET — active ignores for a week. `?week_start=YYYY-MM-DD` (required). */
 ...
 *
 * Re-filing an already-active ignore is a no-op that returns the existing row
 * (see insertRateExemption) — a double-click must not stack duplicates that
...
export async function POST(req: Request) {
  ...
  const { row, error } = await insertRateExemption({
    weekStart,
    name: str(body.name) ?? "",
  ...
  if (error || !row) {
    return NextResponse.json({ error: error ?? "Could not file the ignore" }, { status: 400 });
  }

  void insertAuditLog({
    user_name: authz.sessionEmail,
    user_role: authz.roles[0] ?? "accounting",
    action: "payroll.rate.exempted",
    resource: "payroll_rate_exemptions",
    resource_id: row.id,
    details: {
      person: row.name,
...
/** DELETE — undo an ignore (soft delete). Body: { id }. */
export async function DELETE(req: Request) {
  ...
  const { row, revoked, error } = await revokeRateExemption(id, authz.sessionEmail);
  ...
  void insertAuditLog({
    user_name: authz.sessionEmail,
    user_role: authz.roles[0] ?? "accounting",
    action: "payroll.rate.exemption_undone",
    resource: "payroll_rate_exemptions",
    resource_id: id,
    details: {
