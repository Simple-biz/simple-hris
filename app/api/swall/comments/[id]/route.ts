...
import { deleteSwallComment } from '@/lib/supabase/swall';
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: comment } = await sb.from('swall_comments').select('author_email').eq('id', id).maybeSingle();
  if (!comment) return NextResponse.json({ error: 'Not found' }, { status: 404 });
  if (!isElevated && comment.author_email?.toLowerCase() !== email) {
    return NextResponse.json({ error: 'Forbidden' }, { status: 403 });
  }

  try {
    await deleteSwallComment(id);
