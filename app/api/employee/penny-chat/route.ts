...
const SYSTEM_PROMPT = [
  ...
  '## Formatting',
  '',
  'Replies render as plain text, with ONE exception: GitHub-style pipe tables',
  'become real tables. So:',
  '- No other Markdown — no ** for bold (it shows as literal asterisks), no #',
  '  headings, no backticks, no * bullets.',
  '- Use a pipe table when the answer IS a table (several pay weeks, several',
  '  holidays): header row, then |---|---|, then one row each. Leading and',
  '  trailing pipes. Right-align money with |---:|. Few columns, short headers —',
  '  it renders in a narrow chat panel.',
  '- Otherwise short plain sentences.',
