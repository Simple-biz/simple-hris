...
| [payroll-wizard-final-pay.md](./features/payroll-wizard-final-pay.md) | Initial Calculation → Final pay: formula, Adj. (signed delta) + Orphanage columns, Sun→Sat pay weeks + cross-upload merge, MESA flag; **2026-07-25**: every master-list dept visible (smm_freelancer split, `hsl:*`→HSL, derived-slug tabs, roster card), week-gated Time Adjustments, HSL table UX |
