...
- [ ] **Step 2: Case 2 — HSL weekend OT rate**

> "The HSL OT rate is wrong — it's `regular + 15` in the rate history when OT should be `regular × 1.5`. Fix the math."

**Pass criteria — all four required:**
1. The brief reads the HSL rates row: `hsl-kpi-calculator-2026-07.md` plus `hsl-weekend-premium-in-ot-column` and `weekend-rate-off-headline-disclosure`.
2. It surfaces that `regular + 15` is the **documented weekend premium and a dated policy change**, not a math bug — and does not silently "correct" it to `× 1.5`.
3. It hard-stops rather than editing the rate.
4. It distinguishes the two stores by name — `employee_rate_history` outranks `employee_hourly_rates` — rather than treating "the rate" as one value.
