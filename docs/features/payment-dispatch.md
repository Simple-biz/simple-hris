...
> **Retired 2026-08-07: the USD card + tab.** There used to be a matching **USD** bucket carving out USD-denominated people (US-based staff) the same way. It was removed — a USD row already leads with its USD figure in the normal worksheet, exactly like a PHP row does (see §3.4), so the separate bucket only hid those people from the processor queues where their Bank Preferred routing lives. They now ride the normal rails.
...
- **Carve-out, no double-pay.** `PayrollDispatch.tsx` splits `pending` into two **exclusive** buckets: `copPending` (`payCurrency === 'COP'`) and `mainPending` (everything else, PHP **and** USD). The processor cards, their counts, `totalPending`, and the **All pending** tab all run off `mainPending`, so a COP person appears in **exactly one** place — the COP tab. The COP card only renders when `copPending.length > 0`. All are marked paid through the same `MarkPaidDialog` → `POST /api/payment-dispatches` flow (the record carries `amount_usd` + `amount_php` + `amount_cop`, added by `add_cop_currency.sql`).
