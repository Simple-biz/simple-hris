...
| `pabPayoutForWeek(weekStart, weekEnd, overrides, manualEnd)` | `{ pays, year, month, periodEnd }` — does this paycheck CARRY the PAB (the week after the period closes, Kane 2026-09-08), and for which month? |
...
**Retired 2026-08-12: the Reports tab.** `DispatchReports.tsx`, its `'reports'` rail card and sidebar entry, and every `/api/payment-dispatches/reports/*` route were deleted. The table, its sync triggers, and the surviving library functions (§6.5.5) are untouched — CEO Financial Reports, the overview KPIs, Penny, People payroll history, and the urgent buckets still read them. Seeding is now **automatic at ingest**: both ingest paths — `POST /api/hubstaff-hours` (manual CSV upload) and `run-weekly-sync.ts` (the weekly auto-sync cron) — call `seedMissingDisbursementRecords({ sourceFiles: [fileName] })` best-effort after the hours land. It never fails the upload/sync. Re-ingesting the SAME filename re-seeds it (corrected hours refresh the estimates, paid state is preserved, a partial earlier seed heals); non-weekly uploads (backfills, time-activity exports, "(2)" duplicates) are refused, and so is a file whose pay week is already seeded under a DIFFERENT filename (double-count guard). This replaces the manual seed step (and the tab's "Seed" button).
