...
- **The n8n pill is GONE (2026-09-16)** — the Dispatch step used to carry a pink badge reading "Paystubs send 1-by-1 from Payment Dispatch · n8n on Mark Paid", linking to the n8n Cloud workspace. Kane: *"remove the text as this is stale already, Payment Dispatch handles the sending now"* — naming the transport on a payroll screen told the clerk nothing she can act on, and "1-by-1" read as a caveat about a pipeline that has been the normal path since the per-employee send shipped. **The send itself is untouched**: `/api/payment-dispatches` still calls `forwardPaystubDispatch` per person on `status='paid'` (§ Per-employee send). The green **"Locked in by ‹who› on ‹when›"** stamp stays.
- **The step's prose was CUT BACK (2026-09-16)** — Kane: *"all of them"*, on being shown the four text blocks. The intro paragraph is now the counts alone (`N payable · M excluded`); the heading already says the step locks values and sends to Payment Dispatch, and the per-person email behaviour is documented here rather than restated on screen. The locked card reads **"Values are frozen. Unlocking clears Payment Dispatch in real time"** — the unlock consequence is kept because it is destructive; the procedure is not, the button below names it. The amber rate block is titled **"N rate mismatches"** (the toast on the same step uses the identical wording) over one sentence that KEEPS the money disclosure and its recovery: *"Pay used the dated rate history; the sheet disagrees. If the sheet is right, the difference is owed — settle it with a per-employee Payment Catalog rate."* The table names (`employee_rate_history` / `employee_hourly_rates`) are gone from the UI and live here instead. Per-person lines drop the department and read `Name · ₱paid → ₱sheet · ₱N short`. **Copy only — nothing about the send, the lock, or the rate arithmetic changed, and no disclosure was dropped.**
