...
Since 2026-08-12: after the MESA deposits, both ingest paths (this `POST` and the weekly auto-sync cron) seed the new week's `disbursement_records` via `seedMissingDisbursementRecords({ sourceFiles: [fileName] })` — best-effort, a seed failure never fails the upload/sync. The seeder's gates skip already-seeded files and non-weekly uploads (backfills, time-activity exports, "(2)" duplicates), so re-uploads never recompute a seeded week. The `POST` response carries the row count as `seeded` (`null` = seed failed). This replaced the manual "Seed" button on the removed Payment Dispatch Reports tab.
