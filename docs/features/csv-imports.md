...
Since 2026-08-12: after the MESA deposits, both ingest paths (this `POST` and the weekly auto-sync cron) seed the new week's `disbursement_records` via `seedMissingDisbursementRecords({ sourceFiles: [fileName] })` — best-effort, a seed failure never fails the upload/sync. Re-ingesting the SAME filename re-seeds it (corrected hours refresh the estimates, paid state is preserved, a partial earlier seed heals); non-weekly uploads (backfills, time-activity exports, "(2)" duplicates) are refused, and so is a file whose pay week is already seeded under a DIFFERENT filename (double-count guard). The `POST` response carries the row count as `seeded` (`null` = seed failed). This replaced the manual "Seed" button on the removed Payment Dispatch Reports tab.
