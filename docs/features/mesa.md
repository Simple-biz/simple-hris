...
> **Status:** Requests flow shipped 2026-06-01; contribution ledger backfilled + surfaced 2026-06-26; roster-grounded Non Members (never-joined: not opted in, no start date) / Active Members tabs with bulk Opt In/Out + Requests bulk review, the temporary manual-enrollment bridge, an Active-sheet membership seed, and the `mesa_notes` internal notes log shipped 2026-07-16. **Per-stint MESA accounts** (`mesa_accounts` + `YY-MM-#####` account numbers, opt-out closes/zeroes the account, re-opt-in opens a fresh one) also shipped 2026-07-16; `references/sql/migrate/2026-07-16_mesa_accounts.sql` and the `scripts/seed-mesa-accounts.mjs` seed are **both done** — verified against production 2026-08-11 by `scripts/audit-pending-migrations.mts` (`mesa_accounts` held 307 rows and `employee_hourly_rates.mesa_account_number` exists). Weekly Hubstaff-upload ledger deposits shipped 2026-07-20; the week-delete cascade (deposits + notifications reversed) and the date-only timezone fix shipped 2026-07-25. This doc covers the whole feature. For the accounting-side payout mechanics (Urgent Payments queue, weekly Sun–Sat reconciliation) see [urgent-payments.md](urgent-payments.md); for the underlying tables see [data-sources.md §10 (`mesa_requests`) and §15 (`mesa_ledger`)](../reference/data-sources.md).
...
- **Requests** — the review queue for `opt_out` / `disbursement` / `return` (opt-in is filtered out; that's HR's). Search + status/type filters, paginated (15/page), stat cards (Total / Pending / Approved / Denied). Each pending row → **Review** modal to Approve/Deny with a note. Reviewed rows can be **revoked** (back to pending) or **deleted** — both blocked once `dispatched_at` is set (the money is already sent). A row whose `dispatched_at` is set reads **Approved/Paid** in Status (emerald, wallet icon, tooltip carrying the payout date) rather than plain Approved (2026-07-31), in the queue, the member drill-down and the export alike — `dispatched_at` is stamped by exactly one thing, Send in **Payment Dispatch → Urgent Payments**, and "Approved" on money that already left the fund is what gets a payment chased or made twice. **Bulk**: select rows → bulk Approve / Deny (pending only; approving an `opt_out` also unenrolls) / Delete (skips dispatched).
...
- **Non Members** — everyone **not currently enrolled**: never-joined employees (`mesa_member = false`, no `mesa_member_since`) **and** opted-out ex-members (a trailing opt-out in the ledger, or the flag cleared with a lingering start date), told apart by a per-row **Status** badge (*Never joined* / *Opted out*). Each row has **View** + **Opt In**; **bulk Opt In** via row checkboxes. **Opt In takes an effective date** (2026-09-15 — see "Opt-in effective date" below). Calls `POST /api/toggle-mesa-member` directly. This is a **temporary manual bridge** — added so Accounting can enroll anyone before employees self-serve via the Employee Dashboard's MESA Request tab (which goes through the `mesa_requests` review queue below). Remove this direct-toggle path once that's the primary way members join.
...
**Opt-in effective date** (2026-09-15, Kane's ask): the Non Members **Opt In** dialog carries a required **Effective date** picker, defaulted **at click time** to today in Manila — exactly what the route defaulted to before, so an untouched picker changes nothing — and **not floored at today**: Accounting back-dates corrections, as on the opt-out date. One date applies to every selected row. It travels as `since` on `POST /api/toggle-mesa-member`, which stamps it on **both** `employee_hourly_rates.mesa_member_since` (so the Wizard's ₱100 and the Hubstaff-upload deposit start with the first pay week ending on/after it) and the new `mesa_accounts.opened_on` (so the balance window and the account number's `YY-MM` follow it). **No new column** — both already existed; the UI simply never sent the date. The route now validates at the boundary (`src/lib/mesa/enrollment-date.ts`, browser-safe, tested, imported by the dialog too):

- `since` must be a strict `YYYY-MM-DD` naming a real day → otherwise **400**. The old code sliced any string to ten characters and let Postgres object.
...
**Rule changes are forward-only; a snapshotted week is never rewritten** (Kane, same day: *"make sure we don't change the past values for snapshot things because we added an adjustment on them via the Payroll notes"*). The gate lives in compute code only; nothing re-derives, backfills or migrates a paystub, a `final_pay` snapshot, a staged queue row, a `payment_dispatches` row or a ledger row. What keeps the past intact, each verified: a replay of a past cycle has **Lock In disabled** and `publishFinalPaySnapshot` **returns early** on `isReplay`; the employee stub takes `mesaDeduction` from **any** snapshot on both its fast and slow paths and only recomputes a week that never had one; the Reports step overlays the **saved** `mesa_deduction` on a replay; Payment Dispatch prices from the qualifying snapshot, then the staged row, and freezes paid rows in `payment_dispatches`. Two pre-existing gaps stand outside this change and are logged: `POST /api/paystub-dispatch-queue` has no newest-file guard (the replay protection is UI-only), and re-uploading a **past** week re-deposits by today's membership after the delete cascade rather than by the paid snapshot. The live cycle is live by design — the new gate changes what the current week computes, and the documented re-lock trap ([paystub-dispatch.md](paystub-dispatch.md), 2026-08-26) still applies there, MESA or not.
...
1. **Weekly deduction (−₱100).** Applied automatically to members whose rates row has `mesa_member = true`, but only for pay weeks whose **Friday deposit date** is on/after `mesa_member_since` — `mesaContributesForWeek` in `src/lib/mesa/deposit-date.ts`, Kane's 2026-09-15 ruling (*"Friday should be the deposit dates"*): a Saturday enrollment starts the **following** week, so the ₱400 the upload writes on the Friday is never dated before the account opened. It is **ONE definition** imported by every Wizard deduction site (the final-pay compute and the five display recomputes, which previously ignored the enrollment date), the employee live estimate (`current-pay.ts`), the monthly estimate (`member-monthly-pay.ts`) and the ledger writer — **and who are not currently opted out** (see below). This flag is set by `POST /api/toggle-mesa-member` (from HR opt-in approval) and preloaded from the ledger by `scripts/preload-mesa-membership.mjs`.
...
added 2026-07-20) — a member enrolled **on or before that week's Friday**, the
same `mesaContributesForWeek` the Wizard charges by (2026-09-15), so the
deposit and the deduction can never disagree about a member's first week. It is idempotent per (member, week): re-uploading the same
