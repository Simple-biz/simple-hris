...
**OPEN — a Saturday effective date.** The Wizard charges ₱100 for the pay week ending on that Saturday (`mesa_member_since <= week end`, above) while the deposit writer dates that week's ₱400 on the **Friday before it**, which `summarizeMemberAccount` drops as earlier than `opened_on`; the 2026-08-28 prod backfill used the opposite rule ("first Friday **on or after** the open" — memory `mesa-csv-rebuild-applied`). Two documented rules disagree, the member is charged with nothing visible in their balance, and the picker makes every Saturday selectable (the old default hit it whenever Accounting clicked on a Manila Saturday). Parked for Kane in the Sep 14 session log's Open items; nothing here changes either rule.
