...
- **PostgREST 1000-row cap** *(discovered 2026-07-09)*: PostgREST silently caps un-paginated reads at **1000 rows** (a single `.range(0, 9999)` returns ≤1000 with no error); the active roster is **1075**, so it dropped ~75 people from the transfer picker. Fix = `fetchAllActiveEmployeeRows` pagination; ~44 more un-paginated reads flagged for audit. **Rule:** any read of a table that can exceed 1000 rows after filtering MUST paginate. See [data-sources.md](./data-sources.md).
