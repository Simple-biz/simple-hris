...
The Gift module hub (Orphanage team). Computes every employee's 6-month tenure-gift milestones from their master-list start date, surfaces who is due soon, and is where shipping submissions are reviewed. Hosts the Catalog + Payments sub-tabs. 5-way sub-tab nav: **Roster** (four stat tiles — Gifts owed · Not recorded · Received by someone else · Received; the hero banner and the Tracked / within-1-week / 1-month / 3-month tiles were removed 2026-09-23, and Refresh sits beside the sub-tab nav — then a paginated table sorted by closest upcoming gift, per-row expand to milestone history + editable note + that employee's submissions), **Submissions** (flat list with status filter pills + inline Return / Approve&lock / Edit / Delete; **20 per page** since 2026-09-23 — display only, the counts and the CSV export read the whole filtered list; the page resets on a filter/search change and is clamped when approvals shrink the list), **Recently filled / updated** (`GiftRecentSubmissions`), **Catalog** (`GiftCatalog`), **Payments** (`GiftPayments`). **Approve only locks the row** — it assigns no gift and no price, and nothing becomes a gift payment (stripped 2026-07-14, [[gift-feature-info-only]]); the `deriveGiftForMilestone` this line used to describe no longer exists in `src/` (corrected 2026-09-23).
