...
**Bonus Library columns** *(2026-09-22)*: a bonus assigned in Payment Catalog -> Assignments to an `hsl:<sub>` branch is drawn by `KpiTable` as an extra column after the schema rules — resolved per person by `hslCatalogBonusesFor`, priced by `calcHslCatalogBonus` (`src/lib/hsl-bonus/catalog-bonus.ts`), and folded into `calculated_bonus` by the calculator's `scoreEntry`. Columns are the **union across everyone on the page**, so a per-employee assignment still gets a column and everyone else reads `n/a`. A branch may have **no schema rules at all** and be scored entirely from these columns — `intake_specialist` since 2026-09-22 (`rules: []` + `rulesFromCatalog: true`) and every `data-branch.ts` sub-team. `noKpi` is the trap there: it renders the roster-only card instead, so `KpiTable` never mounts and the Library column disappears with it (`hsl-subdepartments.md` §7d).
