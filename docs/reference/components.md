...
**Accounting → Issues, the time-adjustment row and its dialogs.** Exports `TimeAdjustmentIssueTableRow` (one `<TableRow>` matching the Issues table's eight columns: employee, date, a *Time adjustment* badge + reason, the requested missed time + explanation + who signed, status, what the approval applied, decision, actions) and `TimeAdjustmentIssueDialogs` (View with evidence thumbnails, a lightbox and the decision trail; **Approve / Deny with no hours entry** since 2026-09-15 — approving applies the employee's submitted time ranges and the day total is derived by `approvedAdjustmentDayHours`; Delete for denied rows). JSX only — every rule is a pure, tested function in `src/lib/accounting/issues-time-adjustments.ts`. Decisions call `PATCH /api/time-adjustments/[id]` (`approve` with `approved_hours` / `deny`) and `DELETE /api/time-adjustments/[id]`, the same endpoints the Payroll Wizard panel uses. A row with `stage1_waived_reason = 'manager_filed'` says *Filed by a manager — straight to Accounting* in place of the signer line. `blockedHint` lets the queue pass the reason a viewer cannot act — used for the named exclusions in `src/lib/accounting/time-adjustment-deciders.ts`, which the queue mirrors so the disabled button says why while the server refuses regardless. Doc: [time-adjustment-requests.md](../features/time-adjustment-requests.md) § Accounting → Issues.
...
**Two row kinds since 2026-09-24** (`IssueRow.kind`): `dispute` and `time_adjustment` — time adjustments fetched from `GET /api/time-adjustments` with the statuses `timeAdjustmentStatusesForFilter` maps from the shared filter (Pending → `manager_approved` only), cached per filter under `TAB_CACHE_KEYS.timeAdjustmentIssues`, folded into the four KPI cards by `timeAdjustmentIssueCounts`, and rendered through `TimeAdjustmentIssueRows.tsx`. Rows order time adjustment → dispute. A failed read of either kind shows its own banner and never blanks the other. The third kind, `bank` (Bank Preferred change requests, merged 2026-09-01), was **retired 2026-09-24** with its dialogs, KPI counts and `bankPreferredRequests` cache key: the sending bank is Accounting's alone, set in People → Banking ([bank-preferred-routing.md](../features/bank-preferred-routing.md) §3). `send-from-accounting-only.test.ts` fails if the kind comes back.
...
## `src/components/accounting/SetBankDialog.tsx` *(extracted 2026-09-01; override mode 2026-09-15)*

The one "Set bank" editor shared by Payroll Readiness → Bank Info, Payroll Notes → Offboarded and People → Offboarded. **Default mode:** a resolved effective rail LOCKS the picker and only that rail's missing details are collected; saves via `POST /api/update-employee-ids`, never writes `bank_preferred`. **`override` mode (Offboarded pane only):** the picker is never locked; a "Currently on file" card renders the `current` readout (server-masked — `offboarded-bank-current.ts`); descriptive fields are pre-filled, credentials stay blank with the masked value as placeholder (blank = keep, typed = replace); validation is `validateBankOverride` (typed OR on file) and the save is `PATCH /api/people/[email]/banking` with `buildBankOverridePatch` (both routing columns pinned to the chosen rail, `preferred_bank_slot: 'primary'` when a primary wire field was typed) and `source`. Props: `person`, `prefill?`, `source?`, `warning?`, `override?`, `current?`, `onClose`, `onSaved`. Wiring guards: `src/lib/payroll/offboarded-fixers-override.test.ts`.

## `src/components/accounting/PayrollWizardNotesFab.tsx` — `SetRateDialog` + Offboarded pane *(updated 2026-09-15)*
...
2. **Compensation** — read-only pay info as `CompactStat` blocks (uppercase label, 22px mono number, hint).
   ...
   - **Payout** *(section, 2026-09-12; formerly the `Payment` chip)* — editable disbursement details. Wraps `PreferredPaymentMethodRadios` + `PayoutDetailsFields` with an editorial card frame; toolbar shows current channel as a stamp, plus Edit / Cancel / Save (orange CTA). Read-only when payroll is locked. Its **read** state is the shared `BankCard` (`src/components/banking/bank-card.tsx`), masked to last-4 with an in-card reveal. This section alone waits behind `bankInfoLoaded`; `/api/employee-ids` is never cached.
     - *(2026-09-13)* When the record holds a **second, genuinely different** bank account the read state becomes **`BankCardDeck`** (`src/components/banking/bank-card-deck.tsx`) — the paid card in front, the other tucked behind it, and a button that spins the stack to bring the backup forward. Controlled: the host owns `facingBack` because the Routing row beneath the deck names a slot too. Front card = `pickPreferredBank`; backup = `readBankSlot` (raw, no cross-slot fallback). Flipping is a **viewer**, never a routing change. 151 payees qualify (measured 2026-09-13). See [employee-profile.md](../features/employee-profile.md) §6.2.
     - *(2026-09-13)* The read ⇄ edit swap **elongates** instead of snapping: the body wrapper animates to its measured natural height (`ResizeObserver`) while the two views cross-fade under `AnimatePresence mode="popLayout"`. The wrapper clips **only while travelling** — the bank picker's popover is in-flow `absolute`, not a portal, so a permanent `overflow-hidden` would cut its list off. §6.3.
...
**Bonus Library columns** *(2026-09-22)*: a bonus assigned in Payment Catalog -> Assignments to an `hsl:<sub>` branch is drawn by `KpiTable` as an extra column after the schema rules — resolved per person by `hslCatalogBonusesFor`, priced by `calcHslCatalogBonus` (`src/lib/hsl-bonus/catalog-bonus.ts`), and folded into `calculated_bonus` by the calculator's `scoreEntry`. Columns are the **union across everyone on the page**, so a per-employee assignment still gets a column and everyone else reads `n/a`. A branch may have **no schema rules at all** and be scored entirely from these columns — `intake_specialist` and `filing_specialist` since 2026-09-22 (`rules: []` + `rulesFromCatalog: true`) and every `data-branch.ts` sub-team. `noKpi` is the trap there: it renders the roster-only card instead, so `KpiTable` never mounts and the Library column disappears with it (`hsl-subdepartments.md` §7d).

**Presentation contract** (`impeccable clarify`, same day — Kane on four unlabelled boxes: *"There are no name on the bonus I just assigned, it's confusing"*): the column head is the bonus **name** + a `flat`/`f(x)` chip + the **rule**, and for a formula the rule is the accountant's own expression (clipped, whole text in `title`) — **never the word "formula"**, which named a kind where every neighbouring column names a rule. A `BonusLibraryLegend` strip above the grid states each bonus once in full (name, rule, input names, description in `title`, a `1 person` chip for an employee-scoped one) and reads like the tiered-rate strip beside it. Each formula variable is a **labelled field** (`CatalogVarFields` — name over box, the same treatment `DeptBonusCalculator`'s `VarFields` has always used), revealed when the bonus is ticked; a ticked formula prints **its own peso** under its inputs, **amber at ₱0** so a ticked-but-unfilled bonus is visible. A non-PHP bonus reads `USD -> ₱0` (the card has no FX; see `bonus-catalog.md` §3.1). **`HslBonusEditModal` passes no `catalogFor`, so these columns do not render there** — and its recompute calls `calcBonus` directly rather than `scoreEntry`, which strips the catalog amount on save (session log item 150).
...
**Compare with your sheet** (QC departments, manager mode; `qc-scoring.md` §Compare / Override / Undo): a chip in the card header opens a paste → Compare → Override → Undo panel. **Since 2026-09-14 the pasted sheet is SHARED per dept-week** — Compare `PUT`s it to `/api/qc/compare-paste` (stored in `app_settings['qc.compare_paste.<dept>.<sunday>']`, stamped who/when by the server), opening the panel `GET`s it, prefills an empty box and auto-runs Compare, and an attribution line offers **Use the shared sheet** when the box has drifted. A `destructive` **Delete all** button in the Compare row (inline confirm, `DELETE`, audited with the full text first) removes the shared sheet for everyone and empties the box. `loadSharedPaste` / `saveSharedPaste` / `clearSharedPaste`; the box, result, shared copy and Undo snapshot all reset on week change.
...
Editable catalog of giftable items and anniversary-tier mappings (`GiftTracker` -> Catalog sub-tab). **The free-form Suggestions card was removed 2026-09-23 (Kane)**; the stored `suggestions` array is untouched and round-trips unchanged on Save. `CatalogPayload = { items, anniversaries, suggestions }`; seeds `DEFAULT_PAYLOAD` when empty. `dirty` = JSON diff. Data: `GET /api/gift-catalog`, `PUT /api/gift-catalog`. **An Anniversary Gifts tier's gift is PICKED from Gift items, never typed** (Kane, 2026-09-23): a multi-pick of the distinct item NAMES (`catalogItemNames` — sizes/variants collapse, so a tier sends "Tshirt", the size comes from the submission). The tier stores `gift_items: string[]` plus `gift` = those names joined `" & "` for legacy readers, always written together via `tierGiftFields` (`src/lib/gift-tracker/anniversary-items.ts`, tested). A tier saved before that has only `gift`, split on `" & "`. **A name no longer in Gift items is KEPT and flagged amber (`missingTierItems`), never dropped on load or save** — it can be unticked, not newly added. Live 2026-09-23 three tiers named off-catalog gifts: "Polo" (30 mo), "Speaker" (catalog has Square/Circle), "Lamp" (42 mo). Nothing downstream reads the tier today: approval no longer derives a gift ([[gift-feature-info-only]]); the `deriveGiftForMilestone` this entry used to name is gone from `src/`. **Gift items show an editable `Price (PHP)` column** bound to `price_php` (restored 2026-09-23 on Kane's ask, after its 2026-07-14 removal). It is **reference only**: what an item costs. It never reaches approval, the shipping export or payroll ([gift-tracker-shipping-export.md](../features/gift-tracker-shipping-export.md) § *Tenure gifts carry no price*). The Anniversary Gifts table's USD estimate stays hidden. **Gift items has a `Sized` toggle** (2026-09-23): apparel whose invoice line prints the employee's shirt size; written to every row sharing the item name; absent ⇒ inferred by `isSizedItem` (narrowly — Small/Medium/Large alone is NOT sized, that is the Tote Bag). **The price's one consumer is the Orders invoice** (snapshotted at lock).
...
The Gift module hub (Orphanage team). Computes every employee's 6-month tenure-gift milestones from their master-list start date, surfaces who is due soon, and is where shipping submissions are reviewed. Hosts the Orders, Catalog + Payments sub-tabs. Sub-tab nav: **Roster** (four stat tiles — Gifts owed · Not recorded · Received by someone else · Received; the hero banner and the Tracked / within-1-week / 1-month / 3-month tiles were removed 2026-09-23, and Refresh sits beside the sub-tab nav — then a paginated table sorted by closest upcoming gift, per-row expand to milestone history + editable note + that employee's submissions), **Submissions** (flat list with status filter pills + inline Return / Approve&lock / Edit / Delete; **20 per page** since 2026-09-23 — display only, the counts and the CSV export read the whole filtered list; the page resets on a filter/search change and is clamped when approvals shrink the list), **Recently filled / updated** (`GiftRecentSubmissions`), **Orders** (`GiftOrders`, third, after Submissions — [gift-tracker-orders.md](../features/gift-tracker-orders.md)), **Catalog** (`GiftCatalog`), **Payments** (`GiftPayments`). **Approve only locks the row** — it assigns no gift and no price, and nothing becomes a gift payment (stripped 2026-07-14, [[gift-feature-info-only]]); the `deriveGiftForMilestone` this line used to describe no longer exists in `src/` (corrected 2026-09-23).
...
### Penny AI — the shared assistant widget *(documented 2026-09-12)*

One widget, three mounts, **not a fork**. `CeoChatBubble.tsx` (the floating bubble), `BizAiTab.tsx` (the CEO's full-page tab) and `AdminPennyConsole.tsx` (the Admin tab) all drive `use-ceo-chat.ts`, differing only by `endpoint` (+ `feedbackEndpoint`, `extraBody`, `onQuotaHeader`). Governing docs: [admin-penny-console.md](../features/admin-penny-console.md), [admin-penny-tools.md](../features/admin-penny-tools.md), [ceo-assistant.md](../features/ceo-assistant.md), [employee-penny-ai.md](../features/employee-penny-ai.md).

- **`src/components/ceo/use-ceo-chat.ts`** — conversation state + streaming for every surface. Reads the response as a `ReadableStream` and concatenates decoded chunks into the last assistant message. Lifts **NUL activity frames** out with `splitFrames()` **here, upstream of the transcript** — therefore upstream of the biz-report fence, the pipe-table pass and the Markdown pass, so no downstream parser ever sees one. Frames are narrowed by `t` before any field is read. The transcript is per-mount and client-only; there is no server-side conversation store.
- **`src/components/ceo/ceo-chat-message.tsx`** — renders one assistant reply. Three layers, outermost first: ```` ```biz-report ```` fences → a download card, GitHub pipe tables → real `<table>`s, then the Markdown subset. **No raw-HTML path** — the parser returns data, the renderer returns elements, and a guard test scans for React's escape hatch by name. Takes `tone` (`penny` | `console`); `penny` holds the original violet class strings **verbatim**, so restyle Admin by editing the `console` entry, never `penny`.
- **`src/lib/penny/chat-markdown.ts`** — the Markdown subset parser. Pure, React-free. Links, images, nested lists and block quotes are deliberately out of scope.
- **`src/lib/penny/chat-tables.ts`** *(2026-09-12)* — pipe-table parsing + column analysis, split out so the test runner can reach it (it only walks `src/**/*.test.ts`, never a `.tsx`). Sizes the grid to the **longest row**, so a row with an unescaped `|` no longer loses its tail; right-aligns a column of figures even when the model omits `|---:|` (a *declared* alignment always wins); lets only long text columns wrap, never a numeric one.
- **`src/components/admin/AdminPennyConsole.tsx`** — the Admin tab as an operator's console. Always dark (a deliberate PRODUCT.md exception), centered terminal window, CRT power-on on every entry, boot banner once per browser session, `/clear` + a command rail, and an activity readout driven **only** by server-named frames.
- **`src/components/ceo/penny-attachments.tsx`** *(2026-09-12)* — the files Penny found, as openable chips, plus the image viewer. A chip costs nothing until clicked: the signed URL is minted at **open** time by `GET /api/admin/penny-chat/attachment`, which is also when the open is audited. Images open in a viewer that borrows the console's own window chrome (title bar, status dot, framing reticle) and is a **real modal** — focus moves in, Tab is trapped, Escape and the backdrop close it, body scroll is locked, focus returns to the chip. Rendered through a **portal to `document.body`**, because the console panel animates on a transform and a `position: fixed` descendant of a transformed ancestor positions against that ancestor. PDFs and other files open in a new tab instead.

### `src/components/audit/AuditLogPanel.tsx`

Admin/accounting activity log viewer (also embedded in System Settings). **Rewritten 2026-09-09** (`ddf4c790`; governing doc [audit-log.md](../features/audit-log.md)). The primary filter axis is the **dashboard** (`SurfaceFilterId` = `all` | an `AUDIT_SURFACES` id): the chips and every row badge are **generated from `src/lib/audit/registry.ts`**, not hand-kept — the old 12 hand-written `CATEGORIES` predicates claimed no `orphanage.*`, `wizard.*`, `dispatch.*`, `documents.*`, `people.*`, `bank_*`, `ticket.*` or `time_adjustment.*` row, so those were badge-less under "All activity". Filtering is **server-side** (`GET /api/audit-log?surface=&actor=&since=&until=&before=&limit=`), paging is keyset via `next_cursor`, and a free-text `search` runs over a stated 2000-event window — an incomplete window renders an amber banner instead of a silent "no results". `formatActionLabel()` still turns raw `audit_log.action` codes into sentences; exports `formatAbsoluteTime`, `formatRelativeTime`, `formatActionLabel`. **"Clear Log" is gone**: the header offers a **retention purge** (`PURGE_DAY_OPTIONS`, default 365 days; the API refuses under 90) that previews the count (`DELETE …?preview=1`) before the admin-only `DELETE /api/audit-log?older_than_days=`, which writes `audit.purged` before deleting. Chip counts were removed on purpose — a count over the loaded window posing as a total was the misreading the rewrite set out to fix. No Realtime (manual Refresh).
...
## `src/components/admin/AdminExternalApiClients.tsx` *(added 2026-09-16, rebuilt 2026-09-17)*

The **Integrations** tab of Admin → Webhooks & Integrations (mounted by `AdminWebhooks.tsx`; it was under Admin → API
tokens for one day). The registry of outside systems that read the Global Master List by REST or MCP with a key we issue.
**Console-plain since 2026-09-17 PM**: title row + one primary action, an endpoints strip (REST · MCP · Auth with copy), a
filter toolbar (state chips with counts, text filter, "Updated … ago", Refresh), ONE flat hairline table, dot-and-word status,
ghost row actions. The list is seeded from the Admin tab cache (`useAdminCachedState(ADMIN_CACHE_KEYS.integrationsClients)`)
