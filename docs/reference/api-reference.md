...
`POST` returns `409` when the employee already has an in-flight transfer, `400` on missing email / a no-op move (equal from-to, **or** a source cell that already satisfies the target, e.g. `Client - VA` → `Client VA`, `transferNoOpDenial`, since 2026-09-25) / non-ISO `proposed_effective_date`, `403` when a non-admin manager's target is not a dept they manage.

`PATCH` actions (all guard against acting on an already-decided row with **`409` `Request already <status>`**):
- **`release`** — source-dept manager consents. Locks the effective date to the requester's `proposed_effective_date` (falls back to today), then applies the dept move **immediately** (the effective date is retained only as the rate-proration anchor, not to defer the label change). If the master-list write fails the row stays `approved` (retry via `apply`) and the response carries `applied: false` with an explanatory `error` at `200`. Requires `requireFeatureEdit('manager','team')` + source-dept manager (or admin), and a non-admin may **never release or decline a request they raised** (`403`, since 2026-09-25 — `transferDecisionDenial`, `src/lib/transfers/transfer-authority.ts`). Audit `department_transfer.released`.
...
Gate: `manager` or `admin` (`401`/`403`). Active `global_master_list` people. **`?purpose=transfer`** (the "Request transfer in" dialog, since 2026-09-25) → everyone, including people in a dept the manager also manages. **Without it** (the KPI calculators' add-external-member pickers and "Add missing as externals") → everyone **except** those in a dept the manager manages, because those callers treat each person returned as external to the team. Admins see everyone either way; `candidatePool` in `src/lib/transfers/transfer-authority.ts`. Returns Name + Department + emails only — **no pay/rate data**. Optional `?q=` (name/department/work-email/personal-email substring) and `?department=` filters; also returns the `departments` list for the filter dropdown. Capped at 200 rows. [route.ts](app/api/manager/transfer-candidates/route.ts)
