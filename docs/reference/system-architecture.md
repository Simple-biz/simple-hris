...
> **A second sign-in path exists and is enabled by default:** a **super-admin impersonation** `CredentialsProvider` that signs you in *as* any `@simple.biz` email given a shared password (`SUPER_ADMIN_PASSWORD`, **default `'super-admin'`**). It bypasses Google entirely, inherits the target's roles, stamps `token.impersonated`, and is audit-logged as `auth.impersonation.signin`. Disable with `SUPER_ADMIN_IMPERSONATION=off`. **⚠ This is BLOCKING item 1 of the EOM company-wide release** ([pre-release-security-readiness.md](../features/pre-release-security-readiness.md)): the default password is committed uncommented in `.env.example`, `SUPER_ADMIN_PASSWORD` is absent from `.env.local`, and the form renders on the public `/login`. **Kane ruled 2026-09-09 that the password path goes away entirely** — impersonation becomes an admin-session action launched from Admin → Global Master List; the `blueprint` brief (session `4b3331cb`) is posted and waits on his Q1–Q3. Until that lands, read this paragraph as describing a live exposure, not a feature. Removing this literal from the docs is in that build's scope.
